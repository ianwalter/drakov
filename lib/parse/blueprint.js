...
module.exports = function (filePath, autoOptions, routeMap) {
  return function (cb) {
    ...
    drafter.parse(data, { type: 'ast' }, (err, result) => {
      if (err) {
        log.error(err)
        return cb(err)
      }
      ...
      // THANKS A LOT APIARY!
      const schemaHasRequired = obj => obj.schema.includes('required')
      const examples = findAllNested(result.ast, undefined, schemaHasRequired)
      examples.forEach(example => {
        const schema = JSON.parse(example.schema)
        schema.required = Array.from(new Set(schema.required))
        example.schema = JSON.stringify(schema)
      })
